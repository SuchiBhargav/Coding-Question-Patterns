...
def merge(intervals):
    ...
    for i in range(1, len(intervals)):
        cur = intervals[i]
        if cur.start <= prevEnd:  # overlapping intervals -> adjust the 'end'
            prevEnd = max(cur.end, prevEnd)
        else:  # Connect and append
            mergedIntervals.append(Interval(prevStart, prevEnd))
            prevStart = cur.start
            prevEnd = cur.end
