...
def diff_ways_to_evaluate_expression_rec(memo, input):
    if input in memo:
        return memo[input]
    ...
    if '+' not in input and '-' not in input and '*' not in input:
        ...
        for i in range(0, len(input)):
            ...
            if not char.isdigit():
                ...
                leftParts = diff_ways_to_evaluate_expression_rec(memo, input[0:i])
                rightParts = diff_ways_to_evaluate_expression_rec(memo, input[i+1:])
